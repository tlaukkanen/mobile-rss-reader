/*
 * RssFeedStoreCompatability1Test.java
 *
...
final public class RssFeedStoreCompatability1Test extends LoggingTestCase {

	public RssFeedStoreCompatability1Test() {
		super(10, "RssFeedStoreCompatability1Test");
	}
	...
	public CompatibilityRssFeed1 feed1StoreTestStrFactory(String mname,
			String step, int nbr,
			String suffix,
			long ldate,
			long litem1Date, long litem2Date) throws Throwable {
		try {
			CompatibilityRssFeed1 feed = new CompatibilityRssFeed1(
				"test" + nbr + suffix, "url" + nbr + suffix, "username" + nbr + suffix, "password" + nbr + suffix);
			Vector vitems = new Vector();
			if (litem1Date > 0L) {
				CompatibilityRssItem1 item1 = new CompatibilityRssItem1(
			...
			if (litem2Date > 0L) {
				CompatibilityRssItem1 item2 = new CompatibilityRssItem1("title" + nbr + suffix + ".2", "link" + nbr + suffix + ".2", "desc" + nbr + suffix + ".2",
						new Date(litem2Date));
				vitems.addElement(item2);
			}
	...
	public RssFeedStoreHolder feedStoreTestStrSub(String mname, int nbr, String suffix,
		...
		try {
			...
			CompatibilityRssFeed1 infoFeed = feed1StoreTestStrFactory(mname,
									"2", nbr, suffix,
									ldate, litem1Date,
									litem2Date);
			assertFalse(mname +
	...
	public void testFeedStoreStr2() throws Throwable {
		...
		try {
			...
			storeStringTestSub(mname, true, true,
					fholder.feed, (CompatibilityRssFeed1)fholder.infoFeed, null);
    ...
    public void storeStringTestSub(final String mname,
			...
			RssItunesFeed feed,
			CompatibilityRssFeed1 infoFeed,
		...
		try {
			...
			//#endif
			if (genFeed) {
				infoFeed = new CompatibilityRssFeed1(storeString);
			}
			//#ifdef DLOGGING
			if (finestLoggable) {logger.finest("storeStringTestSub  " + mname + " infoFeed=" + infoFeed);}
			//#endif
			RssItunesFeed nfeed = new RssItunesFeed(new RssFeed(true,
						encoded, storeString));
			//#ifdef DLOGGING
