...
final public class ExtParser {
	...
	//#ifdef DLOGGING
    private Logger logger = Logger.getLogger("compatibility4.ExtParser");
	...
	/** Create an Itunes feed if at least one of these fields are not
	  their uninitialized values, else return an RssItunesFeedInfo to save on
	  memory. */
    public RssItunesFeedInfo getFeedInstance(RssItunesFeedInfo feed, String language,
	...
	/** Create an Itunes item if at least one of these fields are not
	  ...
	  memory. */
    public RssItunesItemInfo createItem(String title, String link, String desc,
					Date date,
					String enclosure,
		...
		if ((m_author + m_subtitle + m_summary + m_duration).equals("") &&
				(m_explicit == (byte)-1)) {
			RssItunesItemInfo item = new RssItunesItem(title, link, desc, date,
    ...
    public void parseNamespaces(SgmlParserIntr parser)
	throws IOException {
	...
	void parseExtItem(SgmlParserIntr parser, char elemChar,
			String elementName)
	throws IOException {
		String subElem;
