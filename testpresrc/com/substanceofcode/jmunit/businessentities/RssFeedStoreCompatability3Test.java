/*
 * RssFeedStoreCompatability3Test.java
 *
...
final public class RssFeedStoreCompatability3Test extends LoggingTestCase {

	public RssFeedStoreCompatability3Test() {
		super(10, "RssFeedStoreCompatability3Test");
	}
	...
	public CompatibilityRssItunesFeed3 feed3StoreTestStrFactory(String mname,
			String step, int nbr,
			String suffix,
			boolean isItunes, long ldate, String explicit,
			long litem1Date, String explicit1, long litem2Date,
		...
		try {
			...
			if (isItunes) {
				feed.modifyItunes(
					true, "title" + nbr + suffix, "description" + nbr + suffix, "language" + nbr + suffix, "author" + nbr + suffix,
					"subtitle" + nbr + suffix, "summary" + nbr + suffix,
					CompatibilityRssItunesItem3.convExplicit(explicit));
			}
			Vector vitems = new Vector();
			if (isItunes && (litem1Date > 0L)) {
				CompatibilityRssItunesItem3 item1 = new CompatibilityRssItunesItem3(
						...
						new Date(litem1Date),
							...
							"subtitle" + nbr + suffix,
							"summary" + nbr + suffix,
							CompatibilityRssItunesItem3.convExplicit(explicit1),
							"50:00");
				vitems.addElement(item1);
			}

			if (isItunes && (litem2Date > 0L)) {
				CompatibilityRssItunesItem3 item2 = new CompatibilityRssItunesItem3("title" + nbr + suffix + ".2", "link" + nbr + suffix + ".2", "desc" + nbr + suffix + ".2",
						new Date(litem2Date),
							"enclosure" + nbr + suffix + ".2", true,
							true,
							"author" + nbr + suffix + ".2",
							"subtitle" + nbr + suffix + ".2",
							"summary" + nbr + suffix + ".2",
							CompatibilityRssItunesItem3.convExplicit(explicit2),
							"50:20");
				vitems.addElement(item2);
			}
	...
	public RssFeedStoreHolder feedStoreTestStrSub(String mname, int nbr, String suffix,
		...
		try {
			...
			CompatibilityRssItunesFeed3 infoFeed = feed3StoreTestStrFactory(mname,
									"2", nbr, suffix,
									isItunes, ldate, explicit, litem1Date,
									explicit1, litem2Date, explicit2);
			assertFalse(mname +
	...
	public void testFeedStoreStr2() throws Throwable {
		...
		try {
			...
			storeStringTestSub(mname, true, true,
					false,
					fholder.feed, (CompatibilityRssItunesFeed3)fholder.infoFeed, null);
    ...
    public void storeStringTestSub(final String mname,
			...
			RssItunesFeed feed,
			CompatibilityRssItunesFeed3 infoFeed,
		...
		try {
			...
			//#endif
			if (genFeed) {
				infoFeed = CompatibilityRssItunesFeed3.deserialize(encoded,
								storeString);
			}
			//#ifdef DLOGGING
